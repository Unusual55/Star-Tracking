...
def valid_triangles(points1,points2, threshold):
    ...
    t1 = Triangle(Vertex(name="a1",x=x1,y=y1,radius=0,brightness=0),Vertex(name="b1",x=x2,y=y2,radius=0,brightness=0),Vertex(name="c1",x=x3,y=y3,radius=0,brightness=0))
    ...
    t2 = Triangle(Vertex(name="a2",x=x1,y=y1,radius=0,brightness=0),Vertex(name="b2",x=x2,y=y2,radius=0,brightness=0),Vertex(name="c2",x=x3,y=y3,radius=0,brightness=0))
...
def match_stars_ransac(G1, G2, dist_thresh=5, inlier_thresh=0.5, max_iterations=1000, validation_threshold = 25, triangle_sim_threshold= 0.05):
    '''
    ...
    fit1, inliers1 = ransac_line_fit(G1,threshold=dist_thresh,max_iterations=max_iterations, inlier_thresh = inlier_thresh)
    fit2, inliers2 = ransac_line_fit(G2,threshold=dist_thresh,max_iterations=max_iterations, inlier_thresh = inlier_thresh)
...
def main():
    ...
    matchings, G1_transformed = match_stars_ransac(G1 = points1, G2 = points2, inlier_thresh = inlier_thresh, dist_thresh = dist_thresh, max_iterations = max_iterations, validation_threshold=validation_threshold, triangle_sim_threshold=triangle_sim_threshold)
